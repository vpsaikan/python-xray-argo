...
UUID = os.environ.get('UUID', '803a012f-769b-488c-ab25-97d6216f22bd')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'python.xuefo.dpdns.org')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiZDM5N2Y0ZTc3ZTNiMzhhYWY4NmRkZWQ2NjZmMDFkZWYiLCJ0IjoiZWY2NGNkYzgtMDRkZC00MTlkLTkzMDgtYjU5ODRiNDZkOGM4IiwicyI6IllUZGpNVGszWmpFdE0yUm1NaTAwTURCaExUZzROMll0WWpVMk9XVXlORGhtWmpCbCJ9')            # Argo固定隧道密钥,留空即使用临时隧道
ARGO_PORT = int(os.environ.get('ARGO_PORT', '8001'))   # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
CFIP = os.environ.get('CFIP', 'visa.cf.877774.xyz')       # 优选ip或优选域名
CFPORT = int(os.environ.get('CFPORT', '443'))          # 优选ip或优选域名对应端口
NAME = os.environ.get('NAME', 'vlesspy')                   # 节点名称
